...
impl RbxClient {
    ...
    pub async fn get_universe_config_legacy(&self) -> Result<UniverseConfigLegacy> {
        ...
        let parsed: V1ConfigResponse = serde_json::from_str(&body).map_err(|e| {
            anyhow::anyhow!("Failed to parse /v1/configuration: {}\nBody: {}", e, body)
        })?;
    ...
    /// PATCH the universe-level configuration via the legacy endpoint. Handles CSRF.
    pub async fn patch_universe_config_legacy(&self, body: Value) -> Result<()> {
        let cookie = self.cookie_header()?.to_string();
        let url = self.legacy_url(&format!(
            "{}/{}/configuration",
